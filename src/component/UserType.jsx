...
console.log(user);
  const getUserType = async () => {
    if (user?._id !== undefined) {
      try {
        ...
        {
          const { data } = await axios.post("https://xerocode-3fc9.onrender.com/api/v1/usertype/register", { id: user?._id }, { withCredentials: true })
  ...
  async function handleSubmit() {
    ...
    try {
      const { data } = await axios.put("https://xerocode-3fc9.onrender.com/api/v1/usertype/update", {
  ...
  return (
    <div style={{ background: "rgba(194, 218, 251, 0.90)", display: "flex", justifyContent: "center", alignItems: "center", height: "100vh" }}>
      ...
      <div style={{ width: "80%", background: "white", display: "flex", justifyContent: "center", flexDirection: "column", padding: "1rem 0rem 4rem 0rem", borderRadius: "2.1875rem " }} >
        ...
        <div style={{ marginTop: "1.5rem", textAlign: "center" }}><h2>{ user?.nodeId?user?.displayName:user?.name}</h2></div>
        ...
        <div style={{ display: "flex", marginTop: "1.5rem", justifyContent: "space-around", marginLeft: "0.5rem", marginRight: "0.5rem" }}>
          {userTypeArr.map((userType, index) => {
            return (
              <div style={{
                flexBasis: "15%", border: " 1px solid #C0C0C0", borderRadius: "0.3125rem", padding: "0.5rem 1rem 0.5rem 1rem", display: "flex", alignItems: "center", gap: "1rem",
                backgroundColor: index === userTypeSelect ? "#1F64FF" : "", cursor: "pointer", color: index === userTypeSelect ? "white" : "", display: "flex", justifyContent: "center", alignItems: "center"
        ...
        </div>
        {
          userTypeSelect != -1 && (
            <div style={{ margin: "2.5rem auto auto auto", display: "flex", gap: "1rem" }}>
              <input style={{ outline: "none", border: "1px solid #C0C0C0", padding: "0.4rem .2rem 0.4rem 0.8rem" }} placeholder={` Enter ${userTypeArr[userTypeSelect].name}`} />
